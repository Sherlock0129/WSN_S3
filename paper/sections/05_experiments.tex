...
% 全局层：源$\to$CH的接收功率剖面（有/无RIS）
\begin{figure}[!t]
  \centering
  % 上：CH0$\to$CH3 路径（0--3链路）
  \includegraphics[width=.98\linewidth]{sections/figures/ris_cs_ch0_ch3.png}\\[-0.3em]
  % 下：CH1$\to$CH2 路径（1--2链路）
  \includegraphics[width=.98\linewidth]{sections/figures/ris_cs_ch1_ch2.png}
  \vspace{-0.5em}
  \caption{全局层多RIS路径在典型走廊上的接收功率剖面（红虚线：无RIS直达；绿实线：经RIS反射）。上图为CH0$\to$CH3（路径0--3），可以看到在越岭点附近，无RIS直达在首个遮挡处迅速跌落到$\approx-100$\,dBm后“消失”，而多跳RIS链路在RIS1/RIS2附近保持$\approx-30\sim-40$\,dBm的可用功率；下图为CH1$\to$CH2（路径1--2），在中段走廊内同样表现出多RIS双跳对背阴侧簇头的显著增益，验证Q1中“多RIS几何机会路径显著抬升源$\to$CH接收功率”的结论。}
  \label{fig:exp_global_cs}
\end{figure}
